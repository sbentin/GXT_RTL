...
/**
 * This layout positions the container's children in a single horizontal or vertical row. Each component may specify its height and width in
 * pixels or as percentage.
 * 
 * <p/>
...
public class RowLayout extends Layout {
	...
	protected void layoutHorizontal(El target) {
		...
		if (LocaleInfo.getCurrentLocale().isRTL()) {
			...
			for (int i = count - 1; i >= 0; i--) {
				x = layoutHorizontalB(h, pw, x, sTop, i);
			}
		}
		else {
			// some columns can be percentages while others are fixed
	...
	private int layoutHorizontalB(int h, int pw, int x, int sTop, int i) {
	...
	private int layoutHorizontalA(int pw, int i) {
		Component c = container.getItem(i);
