 ...
 * 
 * <p />
 * When not forcing a selection ({@link #setForceSelection(boolean)}) {@link #getValue()} can return null event if the user has typed text
 * into the field if that text cannot be tied to a model from from the combo's store. In this case, you can use {@link #getRawValue()} to
 * get the fields string value.
 * 
 * <p />
 * Combo uses a <code>XTemplate</code> to render it's drop down list. A custom template can be specified to customize the display of the
 * drop down list. See {@link #setTemplate(XTemplate)}.
 * 
 * <p />
 * A custom <code>PropertyEditor</code> can be used to "format" the value that is displayed in the combo's text field. For example:
 * 
 * <pre>
 ...
 * <div>Fires before a list item is selected. Listeners can cancel the action by calling {@link BaseEvent#setCancelled(boolean)}.</div>
 * <ul>
 * <li>field : this</li>
 * </ul>
 * </dd>
 ...
 * <div>Fires before all queries are processed. Listeners can cancel the action by calling {@link BaseEvent#setCancelled(boolean)}.</div>
 * <ul>
 * <li>field : this</li>
 * <li>value : query</li>
 * </ul>
...
public class ComboBox<D extends ModelData> extends TriggerField<D> implements SelectionProvider<D> {
	...
	public class ComboBoxMessages extends TextFieldMessages {
		...
		/**
		 * When using a name/value combo, if the value passed to setValue is not found in the store, valueNotFoundText will be displayed as
		 * the field text if defined.
		 * 
	...
	/**
	 * Execute a query to filter the dropdown list. Fires the BeforeQuery event prior to performing the query allowing the query action to
	 * be canceled if needed.
	 * 
	 * @param q
	 *            the query
	 * @param forceAll
	 *            true to force the query to execute even if there are currently fewer characters in the field than the minimum specified by
	 *            the minChars config option. It also clears any filter previously saved in the current store
	 */
	...
	/**
	 * Select an item in the dropdown list by its numeric index in the list. This function does NOT cause the select event to fire. The list
	 * must expanded for this function to work, otherwise use #setValue.
	 * 
	 * @param index
	...
	/**
	 * Sets whether the combo's value is restricted to one of the values in the list, false to allow the user to set arbitrary text into the
	...
	/**
	 * Sets the minimum number of characters the user must type before autocomplete and typeahead active (defaults to 4 if remote, or 0 if
	 * local).
	 * 
	 * @param minChars
	 */
	 ...
	 * <p>
	 * If you wish to preserve the default visual look of list items, add the CSS class name 'x-combo-list-item' to the template's container
	 * element.
	 * 
	 * @param html
	 *            the html
	...
	/**
	 * The action to execute when the trigger field is activated. Use {@link TriggerAction#ALL} to run the query specified by the allQuery
	 * config option (defaults to {@link TriggerAction#QUERY}).
	 * 
	 * @param triggerAction
	 *            the trigger action
	 */
	...
	/**
	 * True to populate and autoselect the remainder of the text being typed after a configurable delay ({@link #typeAheadDelay}) if it
	 * matches a known value (defaults to false)
	 * 
	 * @param typeAhead
	 */
	 ...
	 * 
	 * When set to false the store gets queried on each expand for the data that should get displayed in the list. If you use a loader, than
	 * each time the ComboBox gets expanded, the server gets asked for the data.
	 * 
	 * You want to do this for example, if you filter the content of this ComboBox against some selection in another field.
	 * 
	 * @param useQueryCache
	 *            the useQueryCache to set
	 */
	...
	/**
	 * Sets the model field used to retrieve the "value" from the model. If specified, a hidden form field will contain the value. The
	 * hidden form field name will be the combo's field name plus "-hidden".
	 * 
	 * @param valueField
