...
/**
 * Class for any {@link BoxComponent} that can contain other components. Containers handle the basic behavior of containing components,
 * namely managing, attaching, and detaching the child widgets.
 * 
 * <p/>
 * When children are added to a container they are not physically added to the DOM of the container. Subclasses are responsible for
 * connecting the child components.
 * 
 * <p/>
...
public abstract class Container<T extends Component> extends BoxComponent {
	...
	/**
	 * Adds a item to the container. Fires the <i>BeforeAdd</i> event before inserting, then fires the <i>Add</i> event after the widget has
	 * been inserted.
	 * 
	 * @param item
	...
	/**
	 * Sets the child's parent to this container. In order to support lazy rendering this method uses JSNI to simply set the child parent
	 * without effecting the attached state of the child.
	 * 
	 * @param child
	 *            the child widget
	 */
	...
	/**
	 * Adds a item into the container. Fires the <i>BeforeAdd</i> event before inserting, then fires the <i>Add</i> event after the widget
	 * has been inserted.
	 * 
	...
	/**
	 * Executes the container's layout. If a layout has not been set a <code>FlowLayout</code> will be used.
	 * 
	 ...
	 * Executes the container's layout. If a layout has not been set a <code>FlowLayout</code> will be used.
	 * 
	 * @param force - true to force the layout call, also if caching things it is not needed
	 * @return - true if layout was executed.
	 */
	...
	/**
	 * Removes the item from the container. Fires the <i>BeforeRemove</i> event before removing, then fires the <i>Remove</i> event after
