...
/**
 * ContentPanel is a component container that has specific functionality and structural components that make it the perfect building block
 * for application-oriented user interfaces. The Panel contains bottom and top toolbars, along with separate header, footer and body
 * sections. It also provides built-in expandable and collapsible behavior, along with a variety of pre-built tool buttons that can be wired
 * up to provide other customized behavior.
 * 
 * Code snippet:
 * 
 * <pre>
 ...
 * <div>Fires before the panel is expanded. Listeners can cancel the action by calling {@link BaseEvent#setCancelled(boolean)}.</div>
 * <ul>
 ...
 * <div>Fires before the panel is collapsed. Listeners can cancel the action by calling {@link BaseEvent#setCancelled(boolean)}.</div>
 * <ul>
 * <li>component : this</li>
 * </ul>
 * </dd>
 ...
 * <div>Fires before a content panel is closed. Listeners can cancel the action by calling {@link BaseEvent#setCancelled(boolean)}.</div>
 * <ul>
 * <li>component : this</li>
 * </ul>
 * </dd>
...
public class ContentPanel extends LayoutContainer implements IconSupport {
	...
	public ContentPanel() {
		...
		// buttonAlign = HorizontalAlignment.START;
	...
	/**
	 * Collapses the panel body so that it becomes hidden. Fires the <i>BeforeCollapse</i> before collapsing, then the <i>Collapse</i> event
	 * after collapsing.
	 */
	...
	/**
	 * Expands the panel body so that it becomes visible. Fires the <i>BeforeExpand</i> before expanding, then the <i>Expand</i> event after
	 * expanding.
	 */
	public void expand() {
		if (rendered) {
	...
	/**
	 * Returns the height in pixels of the framing elements of this panel (including any top and bottom bars and header and footer elements,
	 * but not including the body height). To retrieve the body height see {@link #getInnerHeight}.
	 * 
	...
	/**
	 * Returns the width in pixels of the framing elements of this panel (not including the body width). To retrieve the body width see
	 * {@link #getInnerWidth}.
	 * 
	...
	/**
	 * Returns the height in pixels of the body element (not including the height of any framing elements). For the frame height see
	 * {@link #getFrameHeight}.
	 * 
	...
	/**
	 * Returns the width in pixels of the body element (not including the width of any framing elements). For the frame width see
	 * {@link #getFrameWidth}.
	 * 
	...
	/**
	 * True to display the borders of the panel's body element, false to hide them (defaults to true, pre-render). Only applies to
	 * non-framed panels.
	 * 
	 * @param bodyBorder
	 *            true for a body border
	 */
	...
	/**
	 * Sets the button alignment of any buttons added to this panel (defaults to END, pre-render).
	 * 
	 * @param buttonAlign
	...
	public void setButtonAlign(HorizontalAlignment buttonAlign) {
		...
		switch (buttonAlign) {
			case LEFT: {
				if (LocaleInfo.getCurrentLocale().isRTL())
					this.buttonAlign = HorizontalAlignment.END;
				else
					this.buttonAlign = HorizontalAlignment.START;
				break;
			}
			case RIGHT: {
				if (LocaleInfo.getCurrentLocale().isRTL())
	...
	/**
	 * True to make the panel collapsible and have the expand/collapse toggle button automatically rendered into the header tool button
	 * area, false to keep the panel statically sized with no button (defaults to false, pre-render).
	 * 
	 * @param collapsible
	...
	/**
	 * True to create the footer element explicitly, false to skip creating it (pre-render). By default, when footer is not specified, if
	 * one or more buttons have been added to the panel the footer will be created automatically, otherwise it will not.
	 * 
	 * @param footer
	 *            the footer state
	 */
	...
	/**
	 * True to create the header element explicitly, false to skip creating it (defaults to true, pre-render). By default, when header is
	 * not specified, if a {@link #setHeading(String)} is set the header will be created automatically, otherwise it will not. If a title is
	 * set but header is explicitly set to false, the header will not be rendered.
	 * 
	 * @param headerVisible
	 *            true to show the header
	 */
	...
	/**
	 * True to display an interior border on the body element of the panel, false to hide it (defaults to true, pre-render). This only
	 * applies when {@link #setBodyBorder(boolean)} == true.
	 * 
	 * @param insetBorder
	 *            true to display the interior border
	...
	/**
	 * True to allow expanding and collapsing the panel (when {@link #collapsible} = true) by clicking anywhere in the header bar, false to
	 * allow it only by clicking to tool button (defaults to false, pre-render).
	 * 
	 * @param titleCollapse
	 *            the titleCollapse to set
	 */
