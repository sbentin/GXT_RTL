...
public class ToolBar extends Container<Component> {
		...
		switch (alignment) {
			case LEFT: {
				if (LocaleInfo.getCurrentLocale().isRTL())
					this.alignment = HorizontalAlignment.END;
				else
					this.alignment = HorizontalAlignment.START;
				break;
			}
			case RIGHT: {
				if (LocaleInfo.getCurrentLocale().isRTL())
	...
	protected void onRender(Element target, int index) {
		...
		else if (alignment.equals(HorizontalAlignment.END)) {
			if (getItemCount() == 0 || getItemCount() > 0) {// && !isFillItem)) {
				boolean state = layoutOnChange;
				layoutOnChange = false;
				if (LocaleInfo.getCurrentLocale().isRTL())
					insert(new FillToolItem(), this.getItemCount());
				else
