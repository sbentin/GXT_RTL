 ...
 * <div>Fires after an item is selected. Listeners can cancel the action by calling {@link BaseEvent#setCancelled(boolean)}.</div>
 * <ul>
 ...
 * <div>Fires before a item is added or inserted. Listeners can cancel the action by calling {@link BaseEvent#setCancelled(boolean)}.</div>
 * <ul>
 * <li>container : this</li>
 ...
 * <div>Fires before a item is removed. Listeners can cancel the action by calling {@link BaseEvent#setCancelled(boolean)}.</div>
 * <ul>
 * <li>container : this</li>
...
public class TabPanel extends Container<TabItem> {
	...
	// the position where the edge of possible scrolling
	...
	/**
	 * Adds a tab item. Fires the <i>BeforeAdd</i> event before inserting, then fires the <i>Add</i> event after the widget has been
	 * inserted.
	 * 
	 * @param item
	...
	/**
	 * Handle the component event, scrolling left or right on the tab panel
	 */
	...
	/**
	 * Removes the tab item. Fires the <i>BeforeRemove</i> event before removing, then fires the <i>Remove</i> event after the widget has
	 * been removed.
	 * 
	 * @param item
	...
	public void scrollToTab(TabItem item, boolean animate) {
		if (item == null)
			return;
		// pos is the edge position where the unseen part of the tab bar starts
		int barWidth = stripWrap.getWidth();
		int barLeft = stripWrap.dom.getAbsoluteLeft();
		int rightMostPosition = stripLeft + bar.dom.getAbsoluteLeft();
		int scrollSize = stripLeft - bar.getWidth() + 18;
		int edgeOfVisible = barLeft + barWidth;
		int pos = getScollPos();

		El itemEl = item.header.el();
		int left = itemEl.dom.getAbsoluteLeft();
		int right = left + itemEl.getWidth();

		if (LocaleInfo.getCurrentLocale().isRTL()) {
			// scrolling is from highest 0 to lowest scrollSize * -1)
			if (right > edgeOfVisible) {
				// right side is outside the visible scope
				int scrollTo = Math.max(0, rightMostPosition - right);
				scrollTo(scrollTo, animate);
			}
			else if (left <= barLeft) {
				// left side is outside the visible scope
				int scrollTo = Math.min(scrollSize, (left - barLeft));
				scrollTo(scrollTo, animate);
			}
		}
		else {
			// scrolling is from highest scrollSize to lowest 0)
			if (right > edgeOfVisible) {
				// right side is outside the visible scope
				int scrollTo = right - edgeOfVisible;
				if (scrollTo <= tabWidth)
					// this may happen when right is outside and left is inside the visible area.
					scrollTo = rightMostPosition - pos + scrollTo;
				scrollTo(scrollTo, animate);
			}
			else if (left <= barLeft) {
				// left side is outside the visible scope
				int scrollTo = barLeft - left;
				if (scrollTo > 0)
					// this may happen when left is outside and right is inside the visible area.
					scrollTo = rightMostPosition - pos - scrollTo;
				scrollTo(scrollTo, animate);
			}
		}
	...
	/**
	 * True to animate tab scrolling so that hidden tabs slide smoothly into view (defaults to true). Only applies when {@link #tabScroll} =
	 * true.
	 * 
	 * @param animScroll
	...
	/**
	 * True to render each child tab item when it accessed, false to render all (defaults to true). Setting to false would be useful when
	 * using forms as validation would need to be applied to all children even if they had not been selected.
	 * 
	...
	/**
	 * True to automatically resize each tab so that the tabs will completely fill the tab strip (defaults to false). Setting this to true
	 * may cause specific widths that might be set per tab to be overridden in order to fit them all into view (although
	 * {@link #minTabWidth} will always be honored).
	 * 
	 * @param resizeTabs
	 *            true to enable tab resizing
	 */
	...
	/**
	 * Sets the number of pixels to scroll each time a tab scroll button is pressed (defaults to 100, or if {@link #setResizeTabs(boolean)}
	 * = true, the calculated tab width). Only applies when {@link #setTabScroll(boolean)} = true.
	 * 
	...
	/**
	 * Sets the selected tab item. Fires the <i>BeforeSelect</i> event before selecting, then fires the <i>Select</i> event after the widget
	 * has been selected.
	 * 
	 * @param item
	...
	/**
	 * Sets the position where the tab strip should be rendered (defaults to TOP, pre-render). The only other supported value is BOTTOM.
	...
	/**
	 * True to enable scrolling to tabs that may be invisible due to overflowing the overall TabPanel width. Only available with tabs on
	...
	protected void onRender(Element target, int index) {
		...
		strip = stripWrap.createChild("<ul class='x-tab-strip x-tab-strip-" + pos + "'></ul>");
	...
	private void autoScrollTabs() {
		...
		if (barLeft <= edge.dom.getAbsoluteLeft() && edge.dom.getAbsoluteLeft() <= visibleEdgePosition
				&& (stripLeft + barLeft) <= visibleEdgePosition) {
	...
	private int getScollPos() {
		return edge.dom.getAbsoluteLeft() + edge.getWidth();
	}
	...
	private void onScrollLeft() {
		...
		int barLeft = bar.dom.getAbsoluteLeft();
		if (LocaleInfo.getCurrentLocale().isRTL()) {
			int edgeOfVisible = stripWrap.dom.getAbsoluteLeft() + stripWrap.getWidth();
			int scrollSize = (stripLeft + bar.dom.getAbsoluteLeft() - edgeOfVisible) * -1;
			s = Math.max(scrollSize, (barLeft + scrollSize) - pos - getScrollIncrement());
			if (s >= scrollSize) {
				scrollTo(s, getAnimScroll());
			}
		}
		else {
			s = Math.max(0, (stripLeft + barLeft - pos) - getScrollIncrement());
			if (s >= 0) {
				scrollTo(s, getAnimScroll());
			}
		}
	...
	private void onScrollRight() {
		int pos = getScollPos();
		int visibleEdgePosition = stripWrap.dom.getAbsoluteLeft() + stripWrap.getWidth();
		int scrollSize = stripLeft + bar.dom.getAbsoluteLeft() - visibleEdgePosition;
		int barLeft = bar.dom.getAbsoluteLeft();

		int s;
		if (LocaleInfo.getCurrentLocale().isRTL()) {
			s = Math.min(0, ((barLeft - scrollSize) - pos) + getScrollIncrement());
			if (s <= 0)
				scrollTo(s, getAnimScroll());
		}
		else {
			s = Math.min(scrollSize, (stripLeft + barLeft - pos) + getScrollIncrement());
			if (s <= scrollSize) {
				scrollTo(s, getAnimScroll());
			}
		}
	...
	private void updateScrollButtons() {
		int pos = getScollPos();
		int barLeft = bar.dom.getAbsoluteLeft();
		int visibleEdgePosition = barLeft + bar.getWidth();
		int scrollSize = stripLeft + barLeft - visibleEdgePosition;
		...
		if (LocaleInfo.getCurrentLocale().isRTL()) {
			if (GXT.isIE) {
				...
				scrollLeft.setStyleName("x-tab-scroller-left-disabled", pos >= barLeft + 4);
				scrollRight.setStyleName("x-tab-scroller-right-disabled", pos <= barLeft - scrollSize + 4);
			...
			scrollLeft.setStyleName("x-tab-scroller-left-disabled", pos >= stripLeft + barLeft - 4);
			scrollRight.setStyleName("x-tab-scroller-right-disabled", pos <= visibleEdgePosition - 4);
		}
	}
}
