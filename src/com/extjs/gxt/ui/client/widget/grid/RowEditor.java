...
/**
 * This RowEditor should be used as a plugin to {@link Grid}. It displays an editor for all cells in a row.
 * 
 * <dl>
 * <dt><b>Events:</b></dt>
 * 
 * <dd><b>BeforeEdit</b> : RowEditorEvent(rowEditor, rowIndex)<br>
 * <div>Fires before row editing is triggered. Listeners can cancel the action by calling {@link BaseEvent#setCancelled(boolean)}.</div>
 * <ul>
 ...
 * <div>Fires right before the model is updated. Listeners can cancel the action by calling {@link BaseEvent#setCancelled(boolean)}.</div>
 * <ul>
 * <li>rowEditor : this</li>
 * <li>rowIndex : the row index of the row about to be edited</li>
...
public class RowEditor<M extends ModelData> extends ContentPanel implements ComponentPlugin {
	 ...
	 * 
	 * @param monitorPoll
	 *            the polling interval in ms in that validation is done
	 */
	 ...
	 * 
	 * @param monitorValid
	 *            true to monitor this roweditor
	 */
	...
	public void startEditing(int rowIndex, boolean doFocus) {
		...
		for (int i = 0, len = cm.getColumnCount(); i < len; i++) {
			if (LocaleInfo.getCurrentLocale().isRTL()) {
				Field<Object> f = (Field<Object>) getItem(len - (i + 1));
				setFieldValue(f, cm, i);
			}
			else {
		...
		el().scrollIntoView((Element) grid.getView().getEditorParent(), false, new int[] { renderButtons ? btns.getHeight() : 0, 0 });
	}

	private void setFieldValue(Field<Object> f, ColumnModel cm, int i) {
	 ...
	 * 
	 * @param saveChanges
	 *            true to save the changes. false to ignore them.
	 */
	public void stopEditing(boolean saveChanges) {
		...
		for (int i = 0, len = cm.getColumnCount(); i < len; i++) {
			if (!cm.isHidden(i)) {
				Field<?> f = null;
				if (LocaleInfo.getCurrentLocale().isRTL())
					f = (Field<?>) getItem(len - (i + 1));
				else
					f = (Field<?>) getItem(i);
				if (f instanceof LabelField) {
					continue;
				}
				hasChange = hasChange ? hasChange : checkFieldAtEnd(cm, f, i, data);
			}
		}
		RowEditorEvent ree = new RowEditorEvent(this, rowIndex);
		ree.setRecord(record);
	...
	private boolean checkFieldAtEnd(ColumnModel cm, Field<?> f, int i, Map<String, Object> data) {
		String dindex = cm.getDataIndex(i);
	...
	protected void initFields() {
		...
		for (int i = 0, len = cm.getColumnCount(); i < len; i++) {
			ColumnConfig c;
			if (LocaleInfo.getCurrentLocale().isRTL())
				c = cm.getColumn(len - (i + 1));
			else
				c = cm.getColumn(i);
			configField(c, cm, i, len);
		}
		initialized = true;
	}

	private void configField(ColumnConfig c, ColumnModel cm, int i, int len) {		
	...
	private void verifyField(ColumnModel cm, int i, Field<?> f) {
		f.show();
